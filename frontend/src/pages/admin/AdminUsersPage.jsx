    ...
    <>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">View, search, and manage all platform users.</p>
          </div>
          ...
          {['all', 'admin', 'instructor', 'learner'].map((role) => (
            <button key={role} onClick={() => setRoleFilter(role)}
              ...
              <span className={`ml-2 px-1.5 py-0.5 text-[11px] rounded-full font-semibold ${
                roleFilter === role ? 'bg-blue-400 text-white' : 'bg-white/10 text-slate-400'
              }`}>{counts[role]}</span>
            </button>
          ))}
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/5">
                <th className="text-left px-6 py-3.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">User</th>
                <th className="text-left px-6 py-3.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Role</th>
                <th className="text-left px-6 py-3.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                <th className="text-left px-6 py-3.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Joined</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    ...
                    <p className="text-sm mt-1 text-slate-600">Try adjusting your search or filter.</p>
                  </td>
                </tr>
              ) : (
                users.map((u) => (
                  <tr key={u.id} className="hover:bg-white/[0.02] transition-colors duration-150">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ring-2 ring-blue-500/10">
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => toggleActive(u.id)} title={u.is_active ? 'Deactivate' : 'Activate'}
                          className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                            u.is_active ? 'text-orange-400 hover:bg-orange-500/10' : 'text-emerald-400 hover:bg-emerald-500/10'
                          }`}>
                          {u.is_active ? (
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#111827] rounded-2xl shadow-xl w-full max-w-md p-6 animate-in border border-white/5">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white">Add Instructor</h2>
            ...
            <form onSubmit={handleAddInstructor} className="space-y-4">
              {addError && (
                <div className="px-4 py-3 bg-red-500/10 border border-red-500/20 text-red-400 text-[13px] rounded-xl">{addError}</div>
              )}
              <div>
                <label className="block text-[13px] font-medium text-slate-300 mb-1.5">Full Name</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowAddModal(false)}
                  className="flex-1 py-2.5 border border-white/10 text-slate-300 rounded-xl font-medium hover:bg-white/5 transition-all duration-200 cursor-pointer text-[13px]">
                  Cancel
                </button>
                <button type="submit" disabled={addLoading}
                  className="flex-1 py-2.5 bg-blue-500 text-white rounded-xl font-semibold hover:bg-blue-600 transition-all duration-200 cursor-pointer disabled:opacity-50 text-[13px] shadow-sm shadow-blue-500/20">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#111827] rounded-2xl shadow-xl w-full max-w-md p-6 animate-in border border-white/5">
            ...
            <form onSubmit={handleEditUser} className="space-y-4">
              {editError && (
                <div className="px-4 py-3 bg-red-500/10 border border-red-500/20 text-red-400 text-[13px] rounded-xl">{editError}</div>
              )}
              <div>
                <label className="block text-[13px] font-medium text-slate-300 mb-1.5">Full Name</label>
              ...
              <div>
                <label className="block text-[13px] font-medium text-slate-300 mb-1.5">Role</label>
                <select value={editData.role}
                  onChange={(e) => setEditData({ ...editData, role: e.target.value })}
                  className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm text-slate-200 hover:border-white/20 transition-all duration-200">
                  <option value="learner" className="bg-[#111827]">Learner</option>
                  <option value="instructor" className="bg-[#111827]">Instructor</option>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#111827] rounded-2xl shadow-xl w-full max-w-sm p-6 text-center animate-in border border-white/5">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDelete(deleteConfirm)}
                className="flex-1 py-2.5 bg-red-500 text-white rounded-xl font-semibold hover:bg-red-600 transition-all duration-200 cursor-pointer text-[13px] shadow-sm shadow-red-500/20">
                Delete
              </button>
            </div>
          </div>
